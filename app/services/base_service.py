...
class BaseService(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_by_id(self, id: int) -> ModelType:
        ...
        if not result:
            raise HTTPException(
                ...
                detail=f"{self.model.__name__} tidak ditemukan"
            )
        return result
    ...
    async def create(
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Gagal membuat {self.model_name}: {str(e)}"
            )
    ...
    async def update(
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Gagal mengupdate {self.model_name}: {str(e)}"
            )
    ...
    async def delete(self, id: int) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Gagal menghapus {self.model_name}: {str(e)}"
            )
                ...
                if field in obj_data:
                    ...
                    if not is_unique:
                        raise HTTPException(
                            ...
                            detail=f"{field.title()} '{obj_data[field]}' sudah ada"
                        )
