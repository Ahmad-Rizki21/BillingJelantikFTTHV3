...
async def create_pelanggan(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Gagal membuat pelanggan: {str(e)}"
        )
...
async def read_pelanggan_paginated(
    ...
    limit: int = Query(
        15, ge=1, le=100, description="Jumlah items per halaman (maksimal 100)"
    ),
    search: Optional[str] = Query(
        None, description="Filter pencarian berdasarkan nama, email, atau no_telp"
    ),
    ...
    logger.info(
        f"Mengambil data pelanggan dengan pagination: skip={skip}, limit={limit}"
    )
    ...
    try:
        ...
        logger.info(
            f"Berhasil mengambil {len(pelanggan_list)} pelanggan dari total {total_items}"
        )
        return response

    except Exception as e:
        logger.error(
            f"Gagal mengambil data pelanggan dengan pagination: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def read_pelanggan_by_id(pelanggan_id: int, db: AsyncSession = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)):
...
async def update_pelanggan(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Gagal update pelanggan: {str(e)}"
        )
...
async def delete_pelanggan(pelanggan_id: int, db: AsyncSession = Depends(get_db), current_user: UserModel = Depends(get_current_active_user)):
    ...
    try:
        ...
        langganan_stmt = select(LanggananModel).where(
            LanggananModel.pelanggan_id == pelanggan_id
        )
        ...
        invoice_stmt = select(InvoiceModel).where(
            InvoiceModel.pelanggan_id == pelanggan_id
        )
...
async def export_to_csv(
    ...
    query = select(PelangganModel).options(
        ...
        joinedload(PelangganModel.data_teknis),
        joinedload(PelangganModel.langganan).joinedload(LanggananModel.paket_layanan)
    ).order_by(PelangganModel.id.desc()).offset(skip).limit(limit)
    ...
    if not pelanggan_list:
        raise HTTPException(
            status_code=404, detail="Tidak ada data pelanggan untuk diekspor dengan filter yang diberikan."
        )
...
async def import_from_csv(
    ...
    for row_num, row in enumerate(reader, start=2):
        data: Dict[str, Any] = {model_field: row.get(csv_header, "").strip() for csv_header, model_field in column_mapping.items()}
        if not data:
            continue
