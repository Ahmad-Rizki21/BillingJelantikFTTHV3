...
@router.post(
    "/", response_model=InventoryItemTypeSchema, status_code=status.HTTP_201_CREATED
)
async def create_item_type(
    ...
    res = await db.execute(
        select(InventoryItemTypeModel).where(
            InventoryItemTypeModel.name == item_type.name
        )
    )
    if res.scalar_one_or_none():
        raise HTTPException(
            status_code=409, detail=f"Tipe item '{item_type.name}' sudah ada."
        )
...
async def update_item_type(
    type_id: int, item_type: InventoryItemTypeUpdate, db: AsyncSession = Depends(get_db)
):
