...
async function fetchInvoices(isLoadMore = false) {
  if (isLoadMore) {
  ...
  try {
    ...
    // Logika filter status yang disesuaikan
    if (!showPaidInvoices.value) {
      // Jika switch mati, paksa filter hanya 'Belum Dibayar'
      params.append('status_invoice', 'Belum Dibayar');
    } else if (selectedStatus.value) {
      // Jika switch nyala dan ada filter status, gunakan itu
      params.append('status_invoice', selectedStatus.value);
    }
    ...
    if (isLoadMore) {
      invoices.value.push(...newData);
    } else {
      invoices.value = newData;
    }
...
watch([searchQuery, selectedStatus, startDate, endDate, showPaidInvoices], () => {
  applyFilters();
});
