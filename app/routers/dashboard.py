...
async def get_dashboard_data(
    ...
    try:
        ...
        if "view_widget_tren_pertumbuhan" in user_permissions:
            try:
                ...
                dashboard_response.growth_chart = ChartData(
                    labels=[
                        datetime(item.year, item.month, 1).strftime("%b %Y")
        ...
        empty_revenue_summary = RevenueSummary(
            total=0.0,
            periode="bulan",
            breakdown=[]  # Empty BrandRevenueItem list
        )

        empty_invoice_summary = InvoiceSummary(
            labels=[],
            total=[],
            lunas=[],
            menunggu=[],
...
async def get_loyalty_users_by_segment(
    segmen: Optional[str] = None, db: AsyncSession = Depends(get_db)
):
    """
...
async def get_paket_details(db: AsyncSession = Depends(get_db)):
    ...
    for paket_key, details in paket_details.items():
        sorted_lokasi = sorted(
            details.get("lokasi", {}).items(), key=lambda item: item[1], reverse=True
        )
        sorted_brand = sorted(
            details.get("brand", {}).items(), key=lambda item: item[1], reverse=True
        )
    ...
    try:
        ...
        return {
            ...
            "performance_impact": {
                ...
                "utilization_percent": round((pool_status["checked_out"] / (pool_status["pool_size"] + pool_status["overflow"])) * 100, 2)
            }
        }
    except Exception as e:
        return {
...
async def get_cache_statistics(
    ...
    return {
        ...
        "cache_health": {
            "hit_rate_status": "Excellent" if cache_stats["hit_rate_percent"] > 80 else "Good" if cache_stats["hit_rate_percent"] > 60 else "Needs Improvement",
...
async def get_websocket_metrics(
    ...
    return {
        ...
        "performance_analysis": {
            ...
            "message_success_rate": "Excellent" if metrics["success_rate"] > 95 else "Good" if metrics["success_rate"] > 85 else "Needs Improvement",
            "response_performance": "Excellent" if metrics["avg_response_time_ms"] < 50 else "Good" if metrics["avg_response_time_ms"] < 100 else "Slow",
